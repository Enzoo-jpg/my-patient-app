    ...
    try:
        ...
        if "患者姓名" not in df_raw.columns or "所属药房" not in df_raw.columns:
            ...
            with tab1:
                ...
                st.dataframe(view_df[short_cols].style.applymap(highlight_dropped, subset=[c for c in short_cols if "脱落" in c]), use_container_width=True)
                
            with tab2:
                st.markdown("**这里包含购药量、可用库存等全量底层滚动计算轨迹：**")
                st.dataframe(view_df[full_cols].style.applymap(highlight_dropped, subset=drop_cols), use_container_width=True)
            
            # --- 高级 Excel 导出（同步当前的下钻过滤结果） ---
